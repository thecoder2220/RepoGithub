...
DWORD_PTR GetUWorldPtr()
{
	if (!mem)
		return NULL;
	global::pUWorld = mem->RPM<DWORD_PTR>(mem->GetBase() + 0x37D0528, 8);
...
{
	DWORD_PTR fNamePtr = mem->RPM<DWORD_PTR>(mem->RPM<DWORD_PTR>(mem->GetBase() + 0x36D3310, 0x8) + int(ID / 0x4000) * 8, 8);
