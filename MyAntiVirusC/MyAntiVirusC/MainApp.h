...
public:
	...
	CMem(HANDLE hProc) : hProcess(hProc), hModule(nullptr), lpBase(nullptr)
	{
		...
		//on liste tous les processus (32 et 64b)
		if (EnumProcessModulesEx(hProc, hMods, sizeof(hMods), &cb, LIST_MODULES_ALL))
		{
			...
			{
				// on obtient le nom de l'exe sous la forme exemple.exe
				...
				// permet d'initialiser les structures avant de les utiliser
	...
	// le mot-clé template prévient le compilateur que la prochaine chose dont on va lui parler sera générique 
